...
void BitmapsDblBuff::_OnWrite(const uint8_t &index)
{
    ...
    {
        BitBlt(_bitmaps_ctxs[index], 0, 0, _src_size.first, _src_size.second, _src_ctx, 0, 0, SRCCOPY);
    }
    else
    {
    ...
    _use_optimization_flag(use_optimization), _capture_cursor_flag(capture_cursor)
{
    ...
    _bitmap_info.bmiHeader.biWidth = _src_size.first;
    _bitmap_info.bmiHeader.biHeight = -(_src_size.second);/* Make top-down DIB (upper-left corner) */
    _bitmap_info.bmiHeader.biBitCount = 32u;
    _bitmap_info.bmiHeader.biSizeImage = _src_size.first * _src_size.second * 4;
...
std::pair<int, int> BitmapsDblBuff::GetSrcSize()
{
    return _src_size;
}
