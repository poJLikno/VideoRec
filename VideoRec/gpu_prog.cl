__kernel void image_resample(__global unsigned char* dst_y, __global unsigned char* dst_u, __global unsigned char* dst_v, int dst_width, int dst_height, __global unsigned char* src_rgb, int src_width, int src_height)
{
    ...
    b = p1b * d1 + p2b * d2 + p3b * d3 + p4b * d4;
    g = p1g * d1 + p2g * d2 + p3g * d3 + p4g * d4;
    r = p1r * d1 + p2r * d2 + p3r * d3 + p4r * d4;
    ...
    /* Set Y */
    dst_y[y * dst_width + x + 1] = ((66 * r + 129 * g + 25 * b) >> 8);
    ...
    /* Set Y */
    dst_y[(y + 1) * dst_width + x + 1] = ((66 * r + 129 * g + 25 * b) >> 8);
    ...
    /* Set Y */
    dst_y[(y + 1) * dst_width + x] = ((66 * r + 129 * g + 25 * b) >> 8);
}
...
__kernel void image_resample_no_resize(__global unsigned char* dst_y, __global unsigned char* dst_u, __global unsigned char* dst_v, int width, int height, __global unsigned char* src_rgb)
{
    ...
    b = src_rgb[4 * (y * width + x + 1)] * 219 / 255 + 16;
    g = src_rgb[4 * (y * width + x + 1) + 1] * 219 / 255 + 16;
    r = src_rgb[4 * (y * width + x + 1) + 2] * 219 / 255 + 16;
    
    /* Set Y */
    dst_y[y * width + x + 1] = ((66 * r + 129 * g + 25 * b) >> 8);

    /**/
    /* Find pixel #3 */
    /**/

    b = src_rgb[4 * ((y + 1) * width + x + 1)] * 219 / 255 + 16;
    g = src_rgb[4 * ((y + 1) * width + x + 1) + 1] * 219 / 255 + 16;
    r = src_rgb[4 * ((y + 1) * width + x + 1) + 2] * 219 / 255 + 16;

    /* Set Y */
    dst_y[(y + 1) * width + x + 1] = ((66 * r + 129 * g + 25 * b) >> 8);

    /**/
    /* Find pixel #4 */
    /**/

    b = src_rgb[4 * ((y + 1) * width + x)] * 219 / 255 + 16;
    g = src_rgb[4 * ((y + 1) * width + x) + 1] * 219 / 255 + 16;
    r = src_rgb[4 * ((y + 1) * width + x) + 2] * 219 / 255 + 16;
    
    /* Set Y */
    dst_y[(y + 1) * width + x] = ((66 * r + 129 * g + 25 * b) >> 8);
}
